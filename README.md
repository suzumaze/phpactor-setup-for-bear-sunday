...
`phpactorSetup.sourceRepo` 設定の既定値は、今は開発者のローカルパス決め打ちです。対象パッケージが Packagist に公開されたら、正式なバージョン指定に切り替える予定です。

また、セットアップは `phpactor.path` をユーザー設定（User settings）に書き込みます。この拡張でセットアップした後は、他のPHPプロジェクトを開いてもこのphpactorパスが使われるようになります（グローバルインストールしたphpactorを全プロジェクトで共有する設計のため、意図した挙動です）。
